...
* [Boot Measurements](knowledge/boot-measurements.md) — one-hash RTMR1/RTMR2 measurement rules
* [Policy v2 Generation Workflow](knowledge/policy-v2-workflow.md) — JSON and CoRIM enrollment paths
...
Current `one_hash` invariant: RTMR1 measures the stable signer anchor
(root fingerprint + enrolled signer-purpose EKU), not raw certificate-chain
bytes;
RTMR2 measures canonical `policyData` after redacting the re-issuable servTD
...
**Hard rule:** After every push to `<user alias>/MigTD`, immediately launch a
