...
struct Cli {
    ...
    /// CoMID tag version written into the generated mapping
    #[arg(long)]
    tag_version: u64,
...
fn main() -> Result<()> {
    ...
    let payload = build_tcb_mapping(&hash, cli.svn, cli.tag_version)?;
...
fn build_tcb_mapping(hash: &[u8], svn: u16, tag_version: u64) -> Result<Vec<u8>> {
...
mod tests {
    ...
    #[test]
    fn mapping_carries_requested_tag_version() {
        let tag_version = 7;
        let payload = build_tcb_mapping(&[0xA5; 48], 3, tag_version).unwrap();
        let (_, comids) = decode_and_validate_at(&payload, 0).unwrap();
        assert_eq!(comids.len(), 1);
        assert_eq!(comids[0].tag_identity.tag_version_or_default(), tag_version);
    }
}
