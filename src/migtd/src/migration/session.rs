    ...
    poll_fn(|_cx| {
        ...
        if operation == DataStatusOperation::StartMigration as u8 {
            ...
            if REQUESTS.lock().contains(&mig_request_id) {
                ...
                REQUESTS.lock().insert(mig_request_id);
                migration_log!(info, request_id, "Successfully received migration request");
...
pub async fn exchange_msk(info: &MigrationInformation) -> Result<()> {
    ...
    #[cfg(feature = "vmcall-raw")]
    {
        ...
        migration_log!(
            ...
            "Starting MSK exchange using vmcall-raw transport"
        );
        ...
        migration_log!(
            info,
            info.mig_info.mig_request_id,
            "Successfully established vmcall-raw connection"
        );

        // Feature gate: bypass RATLS when test_accept_all is enabled
        #[cfg(feature = "test_accept_all")]
        {
            migration_log!(
                info,
                info.mig_info.mig_request_id,
                "test_accept_all feature enabled - bypassing RATLS"
            );
            ...
            if info.is_src() {
                migration_log!(
                    info,
                    info.mig_info.mig_request_id,
                    "Acting as migration source - sending the local migration exchange information"
                );
                vmcall_raw_instance
                    .send(exchange_information.as_bytes(), 0)
                    .await
                    .map_err(|_| MigrationResult::NetworkError)?;
                migration_log!(
                    info,
                    info.mig_info.mig_request_id,
                    "Source successfully sent the local migration exchange information to destination"
                );

                let size = vmcall_raw_instance
                    .recv(remote_information.as_bytes_mut(), 0)
                    .await
                    .map_err(|_| MigrationResult::NetworkError)?;
                ...
                migration_log!(
                    info,
                    info.mig_info.mig_request_id,
                    "Source successfully received remote migration exchange information from destination"
                );
            } else {
                migration_log!(
                    info,
                    info.mig_info.mig_request_id,
                    "Acting as migration destination - waiting for migration exchange information from the source"
                );
                let size = vmcall_raw_instance
                    .recv(remote_information.as_bytes_mut(), 0)
                    .await
                ...
                migration_log!(
                    info,
                    info.mig_info.mig_request_id,
                    "Destination successfully received remote migration exchange information from source"
                );
                ...
                migration_log!(
                    info,
                    info.mig_info.mig_request_id,
                    "Destination successfully sent migration exchange information to source"
                );
            }
            vmcall_raw_instance
                .shutdown()
            ...
            migration_log!(
                info,
                info.mig_info.mig_request_id,
                "vmcall-raw connection closed successfully"
            );
        ...
        // Default behavior: use RATLS (original AzCVMEmu approach)
        #[cfg(not(feature = "test_accept_all"))]
        {
            migration_log!(
                info,
                info.mig_info.mig_request_id,
                "Using RATLS for mutual attestation and MSK exchange"
            );
            let transport = vmcall_raw_instance;
            ...
            if info.is_src() {
                migration_log!(
                    info,
                    info.mig_info.mig_request_id,
                    "Acting as migration source - establishing TLS client connection"
                );
                // TLS client
                ...
                migration_log!(
                    info,
                    info.mig_info.mig_request_id,
                    "TLS client connection established successfully"
                );
                ...
                migration_log!(
                    info,
                    info.mig_info.mig_request_id,
                    "Successfully sent local exchange information via TLS"
                );
                ...
                migration_log!(
                    info,
                    info.mig_info.mig_request_id,
                    "Successfully received remote migration exchange information via TLS"
                );
                ...
                migration_log!(
                    info,
                    info.mig_info.mig_request_id,
                    "TLS server connection established successfully"
                );
                with_timeout(
                    TLS_TIMEOUT,
                ...
                migration_log!(
                    info,
                    info.mig_info.mig_request_id,
                    "Successfully sent migration exchange information via TLS"
                );
    ...
    // Non-vmcall-raw transports (always use RATLS)
    #[cfg(not(feature = "vmcall-raw"))]
    {
        migration_log!(
            info,
            info.mig_info.mig_request_id,
            "Starting MSK exchange using TLS transport"
        );
        let transport;
    ...
    if info.is_src() {
        ...
        let mut ratls_client =
            ...
            #[cfg(all(feature = "vmcall-raw", not(feature = "test_accept_all")))]
            ratls_client
                .transport_mut()
                .shutdown()
                .await
    ...
    if info.is_src() {
        ...
        Ok(())
    } // Close the #[cfg(not(feature = "vmcall-raw"))] block
}
...
pub fn exchange_info(
    ...
    migration_log!(
        info,
        info.mig_info.mig_request_id,
        "Preparing exchange information for migration which contains MSK and supported version range"
    );
...
fn read_msk(mig_info: &MigtdMigrationInformation, msk: &mut MigrationSessionKey) -> Result<()> {
    migration_log!(
        ...
        "Reading Migration exchange information from TDX module"
    );
    ...
    migration_log!(
        info,
        mig_info.mig_request_id,
        "Successfully read MSK from TDX module"
    );
    Ok(())
}
...
pub fn write_msk(mig_info: &MigtdMigrationInformation, msk: &MigrationSessionKey) -> Result<()> {
    migration_log!(
        info,
        mig_info.mig_request_id,
        "Writing Migration exchange information to TDX module"
    );
    ...
    migration_log!(
        info,
        mig_info.mig_request_id,
        "Successfully wrote MSK to TDX module"
    );
