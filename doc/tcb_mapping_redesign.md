...
- **Rebinding and migration** — the source MigTD cannot map `init_servtd_info_hash` (= `SHA384(TDINFO)`) to an SVN directly, so it must accept the full init TDINFO from the untrusted VMM on every request and re-derive the registers.
...
- **Anti-downgrade binding** — RTMR2 commits to the exact `servtdIdentity` bytes, so a peer booted with an obsolete signed identity gets a different `tdinfo_hash` and falls outside the mapping (fails closed).
- **Updatable TCB mapping** — because `servtdTcbMapping` is redacted, it can be re-signed and swapped into the CFV without rebuilding the IGVM.

# Additional details

## Problem — supporting diagrams and structures
...
This single extend folds together two distinct security properties:
...
sub-tree), and defeating `servtdIdentity` playback / TCB-downgrade attacks
(covered specifically by the inclusion of
`servtdCollateral.servtdIdentity` — see below).

**Why the `servtdIdentity` binding is necessary:**

- Each MigTD uses its locally-loaded `servtdIdentity` (specifically its `tcbLevels`) to translate a peer's `(isvProdId, isvSvn)` into a TCB status during the migration handshake.
- The issuer periodically re-publishes `servtdIdentity` with updated `tcbLevels` to revoke or downgrade old SVNs.
- **Without** this binding, an attacker can boot a malicious peer with an obsolete-but-still-issuer-signed `servtdIdentity` (one from before the revocation) and present old, vulnerable SVNs as `UpToDate`, then migrate from/to a healthy MigTD — a classic playback/downgrade attack.
- **With** this binding, RTMR2 commits to the exact `servtdIdentity` bytes the local MigTD will use. Any other peer (including a malicious one) that boots with a different `servtdIdentity` (obsolete or not) gets a different RTMR2 and therefore a different `tdinfo_hash`. Since the authority only publishes `svnMappings[]` entries for the **current** `servtdIdentity`, the peer falls outside the mapping list and the migration fails closed.
